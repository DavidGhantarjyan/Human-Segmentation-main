...
if __name__ == '__main__':
    ...
    # Directory structure: train_results/model_name
    ...
    if weights_load_from is not None:
        ...
        # Finding the first 'weights.pt' file in the given folder or path.
        last_weights_path = find_model_in_dir_or_path(weights_load_from)
    # If 'weights_load_from' is None or False
    else:
        # If 'create_new_model' is None or False
    ...
    if last_weights_path is not None:
        ...
        # Loads the saved weights and model parameters.
        # 'state_dict' is a dictionary containing all the parameters (weights and biases) of the neural network.
        model.load_state_dict(checkpoint['model_state_dict'])
        # Loads the optimizer state.
        # The optimizer stores information about parameter updates, such as velocities and gradient moments
        # (e.g., for Adam or RMSprop).
        ...
        # If 'last_weights_path' is not found, meaning 'weights.pt' is not found
    ...
    try:
        # 'model_dir' -> e.g., train_results/model_name/2025-02-19_12-00-00/res_10/, the directory where the weights are located.
        # 'last_weights_path' -> e.g., train_results/model_name/2025-02-19_12-00-00/res_10/weights.pt
        # If 'loss_history' or 'accuracy_history' files exist, we load them; if not,
        # we use the 'global_epoch' column from the saved CSV as the index.
        ...
        # If in the loaded 'weights.pt', or more precisely in its directory, there are no seeds, we ignore them.
        # If 'loss' and 'accuracy' files exist but there is no 'weights.pt', nothing is loaded, and that's okay.
        ...
        # The index column is 'global_epoch', and the data column includes 'train_loss'
        loss_history_table = pd.DataFrame(columns=['global_epoch', 'train_loss'])
        # The index column is 'global_epoch', and the data column includes 'train_accuracy'
    ...
    # profiler.disable()
    # pyprof2calltree.convert(profiler.getstats(), 'pre_training_profile.out')
    ...
    for epoch in range(1, do_epoches + 1):
        # 'curr_run_start_global_epoch' is set to checkpoint['epoch'] + 1
        ...
        with train_profiler:
            for batch_idx,  (batch_inputs, batch_targets, mask) in enumerate(
                ...
                with record_function("## CPU Data Transfer ##"):
                    batch_inputs = batch_inputs.to(device,non_blocking=True).detach()
                ...
                with record_function("## GPU Loss Calculation1 ##"):
                    with record_function("## BoundaryLossCalculator ##"):
