...
BLUR_KERNEL = (31, 31)  # Kernel size (unused in this version, retained for compatibility)

# Configure ONNX runtime providers for GPU acceleration
providers = [
    ...
    # }),
    'CUDAExecutionProvider', # Use CUDA for inference
]
...
input_name = session.get_inputs()[0].name # Get input name for ONNX model

# Load static background image
background_img = cv2.imread("eif.png")
...
while True:
    ...
    refined_mask = torch.clip(GuidedFilter(r=5, eps=1e-6)(frame_torch_tensor,
                                                          mask_cleaned_torch_tensor), 0.0, 1.0)

    # -------------------------------
    # Generate Display Image with Background Replacement
    # -------------------------------
    # Convert refined mask to NumPy for blending
    refined_mask_np = refined_mask.squeeze(0).permute(1, 2, 0).cpu().numpy()  # Shape: (height, width, 3)
    refined_mask_np = np.clip(refined_mask_np, 0, 1)  # Ensure mask values are in [0, 1]

    # Perform alpha blending with foreground (frame) and background image
    foreground = (frame.astype(np.float32) * refined_mask_np).astype(np.uint8) # Apply mask to frame
    background = (background_img.astype(np.float32) * (1 - refined_mask_np)).astype(np.uint8) # Apply inverse mask to background
    blended = cv2.add(foreground, background)  # Combine foreground and background
    ...
    # Add FPS text to blended image
    cv2.putText(blended, f"FPS: {fps:.2f}", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2) # Green text for color image

    # Display original frame and blended output
    cv2.imshow("Original Video", frame)
    cv2.imshow("Model Output (Thresholded)", blended)
