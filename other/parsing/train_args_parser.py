...
background_rotation_angle = parse_range(ydict['data']['rotation_settings']['background_rotation_angle'],[-60,60],[-60,60])
foreground_rotation_angle = parse_range(ydict['data']['rotation_settings']['foreground_rotation_angle'],[-60,60],[-60,60])
...
# natural_data_mask_pre_calculation_mode
...
# если saves_count == 0 => saves_count = do_epoches =>
# np.linspace(do_epoches / do_epoches, do_epoches, do_epoches, dtype=int) от 1 до 10, 10 раз
# если saves_count == 1 =>
# np.linspace(do_epoches / saves_count=1, do_epoches, saves_count=1, dtype=int) от 10 до 10, 1 раз
# если saves_count == 2 =>
# np.linspace(do_epoches / saves_count=2, do_epoches, saves_count=2, dtype=int) от 5 до 10, 2 раз
# np.linspace(do_epoches / saves_count=9, do_epoches, saves_count=9, dtype=int) от 10/9 = 1 до 10, 9 раз
...
def model_has_been_saved():
    ...
    try:
        # yaml.dump(ydict, f) преобразует словарь ydict в YAML-формат и записывает его в файл f.
