...
while True:
    ...
    # -------------------------------
    # Generate Grayscale Mask for Display
    # -------------------------------
    # Convert refined mask to grayscale for display
    binary_display = up_scaled_mask_guided.squeeze(0).permute(1, 2, 0).cpu().numpy()
    ...
    # Add FPS text to grayscale mask
    cv2.putText(binary_display, f"FPS: {fps:.2f}", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (255,), 2)  # White text for grayscale image

    # Display original frame and grayscale mask
