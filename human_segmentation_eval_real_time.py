...
ONNX_PATH = 'train_results/UNet_MobileNetv3/UNet_MobileNetv3_fp16.onnx'
...
while True:
    ...
    # CuPy uint8
    mask_cleaned = remove_letterbox_padding(binary_output, pad_info, (orig_h, orig_w)).get()
    # mask_cleaned = apply_guided_filter(frame, mask_cleaned, radius=1, eps=1e-6)
    end_time = time.time()
    fps = 1 / (end_time - start_time)

    cv2.putText(mask_cleaned, f"FPS: {fps:.2f}", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1, 255, 2)
    cv2.imshow("Original Video", frame)
    cv2.imshow("Model Output (Thresholded)", mask_cleaned)
